...
/*	createQueue
	Allocate memory for a queue head node from dynamic
...
/*	emptyQueue
	This algorithm checks to see if queue is empty.
		...
		Return	true if successful; false if queue has data
*/
bool emptyQueue(QUEUE* queue) {
	//Statements
...
/*	fullQueue
	This algorithm checks to see if queue is full. It is
	full if memory cannot be allocated for next node.
		Pre		queue is a pointer to a queue head node
...
/*	destroyQueue
	...
	memory, then delete & recycles queue head pointer.
