...
/*	Header file for binary search tree(BST). Contains
	structural definitional and protype for BST.
*/
typedef struct node {
	void* dataPtr;
	struct node* left;
...
/*	BST_Create
	Allocate dynamic memory for an BST tree head
	node and returns its address to caller
		Pre		compare is address of compare function
				use when two nodes need to be compared
...
/*	BST_Full
	If there is no room for another node, return true.
...
/*	_retrieve
	Searches tree for node containing requested key and
	returns its data to the calling function.
