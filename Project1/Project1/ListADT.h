...
bool traverse(LIST* pList, void* pArgu, void** dataOutPtr);
...
/*	createList
	Allocates dynamic memory for a list head node
	and returns its address to caller
		Pre		compare is address of complete function
...
/*	fullList
	...
	another node.
		...
		Return	boolean true if full; false if room for node
*/
bool fullList(LIST* pList) {
	//Local Definitions
	NODE* temp;
...
/*	_insert
	Inserts data pointer into a new node.
		...
		Post	data have been inseted in sequence
...
/*	_delete
	Deletes data from a list and returns pointer
	to data to calling module.
		Pre		pList pointer to valid list.
...
/*	search
	Searching list and passes back address of node
	containing target and its logical predecessor.
		Pre		pList pointer to initialized list
				pPre pointer variable to predecessor
				pLoc pointer variable to receive node
				pArgu pointer to key being sought
		Post	pLoc points to first eqaul/greater key
